...
def is_inside_parallel(points, polygon):
    ...
    polygon : numpy.ndarray
        Array of (x,y) coordinates defining the polygon vertices. The last vertex
        should be the same as the first to close the polygon.

    Returns
    -------
    numpy.ndarray
        Boolean array of length N where True indicates the corresponding point
        is inside or on the edge of the polygon

    Notes
    -----
    This function parallelizes the point-in-polygon check using Numba's parallel
    processing capabilities. It's particularly efficient for checking large numbers
    of points against the same polygon.

    Examples
    --------
    >>> # Define a square and points to check
    >>> square = np.array([(0,0), (1,0), (1,1), (0,1), (0,0)])
    >>> points = np.array([(0.5, 0.5), (2, 2), (0, 0.5)])
    >>> is_inside_parallel(points, square)
...
def is_inside(polygon, point):
    ...
    This is a helper function for is_inside_parallel.
    ...
    Notes
    -----
    This function uses a ray-casting algorithm that counts the number of times a
    horizontal ray from the point intersects with polygon edges. The function is
    optimized using Numba's @jit decorator.
    ...
    >>> 
    >>> # Check points
    >>> is_inside(square, (0.5, 0.5))  # inside
    1
    >>> is_inside(square, (2, 2))      # outside
    0
    >>> is_inside(square, (0, 0.5))    # on edge
    2
    """
